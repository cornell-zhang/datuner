...
bool AutoTuner::callOpenTuner(Task* task, vector<Result*>& results, int rank) {
  if(_tune_type == 1) _path = "/proj/xsjhdstaff3/changx/project/parTuner/experiment/vpr/vtr_release/vtr_flow/tasks/openTunerTiming/experiment/";
  if(_tune_type == 2) _path = "/proj/xsjhdstaff3/changx/project/parTuner/experiment/vivado-new/";
  if(_tune_type == 3) _path = "/proj/xsjhdstaff3/changx/project/parTuner/experiment/ise/";

  if(_exp_type == 1) _path = _path+"my/";
...
void AutoTuner::param_parse(Task* task, int rank) {
  ...
  for(int i = 0; i < params.size(); i++) {
    ...
    assert(param != NULL);
    ftr<<param->name<<" "<<param->type<<" "<<param->defVal<<" "<<param->min<<"  "<<param->max<<" "<<param->options.size();
...
int AutoTuner::c2py2(vector<Result*>& results,int rank) {
  printf("Going to call OpenTuner\n");
  FILE* file;
  string python_name;
  if(_tune_type == 1) python_name = "tuneVPR.py";
  if(_tune_type == 2) python_name = "tuneVivado.py";
  if(_tune_type == 3) {
    char name[20];
    sprintf(name,"tuneISE%d.py",rank);
    python_name = name;
  }
  string tmp_name = _path+_design+"/"+python_name;
  
  file=fopen(tmp_name.c_str(),"r");
  PyRun_SimpleFile(file,tmp_name.c_str());
  ...
  if(_tune_type == 3) parse_ISE_result(results,rank);
