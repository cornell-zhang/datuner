...
class VIVADOFlagsTuner(MeasurementInterface):
  ...
  def run(self, desired_result, input, limit):
    ...
    cmd='mkdir -p '+self.workspace+'/'+str(self.args.rank)+'/'
    subprocess.Popen(cmd,shell=True).wait()
    cmd='mkdir -p '+self.workspace+'/'+str(self.args.rank)+'/'+str(result_id)+'/'
    subprocess.Popen(cmd,shell=True).wait()
    workdir = self.workspace+'/'+str(self.args.rank)+'/'+str(result_id)+'/'
