...
def RealRESIZE(x, FixP, QType, IDString=None):
  if x > 0 or x == 0:
    ...
    MSB = 1 
  if FixP[0] < 1:
    raise Exception('the bit of Integer should be greater than 0')  
  ...
  maxVal = 2**(FixP[0]-1+FixP[1])-1                       # Calculate the maximum of unsigned <I+F,0>

  # Signed a
  if FixP[2] == 's':
    minVal_clp = -2**(FixP[0]-1+FixP[1])
  ...
  if y > maxVal or y < minVal_sat:
    ...
    # Overflow
      ...
      tmp = 2**(FixP[0] - 1 + FixP[1] + (FixP[2] == 's'))
