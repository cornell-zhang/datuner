...
class SearchDriver(DriverBase):
  ...
  msgPassing=None
  @classmethod
  def setmsg(cls,msgPassing=None):
    """
    message sharing in collaborative distributed exploration
    """
    if msgPassing is not None:
      cls.msgPassing = msgPassing
  ...
  def process_new_results(self):
    if self.msgPassing is not None:
      """
      receive message when it is ready
      """
      buf = []
      self.msgPassing.recvMsg(buf,1)
      if len(buf) != 0:
        """
        deal with newly received message
        """
        tmpresult = float(buf[0]['time']) #optimization objective
        tmpcfg = buf[0]['cfg'] #configuration
        tmpitem = buf[0]['result'] #result shared among tuners
        for ix in buf: #find the minimum objective
          if  tmpresult > float(ix['time']):
            tmpresult = float(ix['time'])
            tmpcfg = ix['cfg']
            tmpitem = ix['result']

        #TODO(Chang): check db
#        tmpitem = None
#        tmpitem =self.session.query(Result).filter(Result.configuration == tmpcfg).first()
#        if tmpitem is None:
#          res = Result()
...
#          tmpitem = res
        if self.best_result is None:
          self.best_result = tmpitem
          tmpitem.was_new_best = True
        elif self.objective.lt(tmpitem, self.best_result):
          self.best_result = tmpitem
          tmpitem.was_new_best = True
    """
    finish manipulating new message
    """

    mytmpresult = None #best result
    ...
    #send out best result
    if mytmpresult is not None:
      msgdata = {}
      msgdata['cfg'] = mytmpresult.configuration.data
      msgdata['time'] = mytmpresult.time
      msgdata['result'] = mytmpresult
      if self.msgPassing is not None:
        self.msgPassing.sendMsg(msgdata,1)
