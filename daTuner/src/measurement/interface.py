...
class MeasurementInterface(object):
  ...
  def run_precompiled(self, desired_result, input, limit, compile_result, id):
    ...
    """
    res = self.run(desired_result,input,limit)
    return res
  ...
  @classmethod
  def main(cls, args, msgPassing=None, *pargs, **kwargs):
    from opentuner.tuningrunmain import TuningRunMain
    tcls= cls(args,*pargs,**kwargs)
    if msgPassing is not None:
      tcls.msgPassing = msgPassing
    return TuningRunMain(tcls, args).main()
