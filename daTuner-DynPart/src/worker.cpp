...
int main(int argc, char** argv) {
  ...
  int share_best = 0; //0:false 1:true
...

  {
     ...
     if(diffdb) {
      ...
      string dbdir="/proj/xsjhdstaff3/changx/project/parTuner/experiment/vivado-new/"+design+"/";
      char dbtmp[20];
  ...
  if(myrank == 1) {
    ...
    AutoTuner* tuner = new AutoTuner(task->subspace->id, design, tune_type, exp_type); //start from 1
    assert(tuner != NULL);
    //usleep(1000000*myrank);
    //if(myrank == 1) 
    tuner->callOpenTuner(task,results,myrank);
